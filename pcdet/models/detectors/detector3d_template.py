...
from .. import backbones_2d, backbones_3d, classifier
...
class Detector3DTemplate(nn.Module):
    ...
    def build_densenet(self, model_info_dict):
        if self.model_cfg.get('CLASSIFIER', None) is None:
            return None, model_info_dict

        densenet_module = classifier.__all__[self.model_cfg.CLASSIFIER.NAME](
            num_classes=len(self.class_names),
            model_cfg=self.model_cfg
        )
        model_info_dict['module_list'].append(densenet_module)
        return densenet_module, model_info_dict
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                ...
                if not isinstance(cls_preds, list):
                    ...
                    multihead_label_mapping = [
                        torch.arange(1, self.num_class, device=cls_preds[0].device)]  # [tensor([0, 1, 2])]
                else:
                ...
                for cur_cls_preds, cur_label_mapping in zip(cls_preds, multihead_label_mapping):
                    ...
                    cur_pred_scores, cur_pred_labels = model_nms_utils.multi_classes_nms(
