...
def class_agnostic_nms(box_scores, nms_config, score_thresh=None):
    # 1.首先根据置信度阈值过滤掉部分box
    ...
    if box_scores.shape[0] > 0:
        ...
        keep_idx, selected_scores = getattr(iou3d_nms_utils, nms_config.NMS_TYPE)(
            box_scores_nms, nms_config.NMS_THRESH, **nms_config
        )
        # 根据返回索引找出box索引值
...
def multi_classes_nms(cls_scores, nms_config, score_thresh=None):
    """
    Args:
