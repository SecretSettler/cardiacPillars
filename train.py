...
def main():
    ...
    output_dir = Path(
        cfg.ROOT_DIR + os.sep + 'output' + os.sep + cfg.EXP_GROUP_PATH + os.sep + cfg.TAG + os.sep + args.extra_tag)
    ...
    train_data = create_data.CardiacDataset('training_set', transform=None, target_transform=None,
                                            loader=create_data.points_reader,
                                            root_path='/home/s2020153/cardiac/DATASET/training_set',
                                            config=cfg)

    val_data = create_data.CardiacDataset('testing_set', transform=None, target_transform=None,
                                          loader=create_data.points_reader,
                                          root_path='/home/s2020153/cardiac/DATASET/testing_set',
                                          training=False,
                                          config=cfg)
    ...
    if dist_train:
        model = nn.parallel.DistributedDataParallel(model, device_ids=[cfg.LOCAL_RANK % total_gpus])
    # 将模型结构记录到日志中
    ...
    # 训练结束后，对模型进行评估
    # 1.构建test数据集和加载器
    test_set, test_loader, sampler = build_dataloader(val_data,
                                                      batch_size=args.batch_size,
                                                      dist=dist_train, workers=args.workers, training=False
                                                      )
    # 2.构造评估结果输出文件夹
