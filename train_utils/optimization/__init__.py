...
def build_optimizer(model, optim_cfg, params_not_trained=None):
    # 如果采用adam优化器，则调用optim的Adam初始化
    if optim_cfg.OPTIMIZER == 'adam':
        if params_not_trained is None:
            optimizer = optim.Adam(model.parameters(), lr=optim_cfg.LR, weight_decay=optim_cfg.WEIGHT_DECAY)
        else:
            optimizer = optim.Adam(params_not_trained, lr=optim_cfg.LR, weight_decay=optim_cfg.WEIGHT_DECAY)
    # 如果采用sdg优化器，则调用optim的sgd初始化
        ...
        if params_not_trained is None:
            ...
            optimizer = optim.SGD(
                params_not_trained, lr=optim_cfg.LR, weight_decay=optim_cfg.WEIGHT_DECAY,
                momentum=optim_cfg.MOMENTUM
            )
